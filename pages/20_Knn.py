...
# Initial Data Plot
st.dataframe(df)
fig = px.scatter(df, x='Feature1', y='Feature2', symbol='Label',
                 symbol_map={'0': 'square-dot', '1': 'circle'})
fig.add_trace(
...
predicted_label, nearest_neighbours, far_neighbours = find_nearest(
    x, y, input, k)

st.title('Prediction')
st.subheader('Predicted Label : {}'.format(predicted_label))

nearest_neighbours = [[neighbour[1], x[neighbour[1], 0], x[neighbour[1], 1],
                       neighbour[0], y[neighbour[1]]] for neighbour in nearest_neighbours]

nearest_neighbours = pd.DataFrame(nearest_neighbours, columns=[
                                  'id', 'Feature1', 'Feature2', 'Distance', 'Label'])
st.dataframe(nearest_neighbours)

far_neighbours = [[neighbour[1], x[neighbour[1], 0], x[neighbour[1], 1],
                   neighbour[0], y[neighbour[1]]] for neighbour in far_neighbours]
far_neighbours = pd.DataFrame(far_neighbours, columns=[
                              'id', 'Feature1', 'Feature2', 'Distance', 'Label'])

fig2 = px.scatter(far_neighbours, x='Feature1', y='Feature2',
                  symbol='Label', symbol_map={'0': 'square-dot', '1': 'circle'})

for index, neighbour in nearest_neighbours.iterrows():
    fig2.add_trace(
        go.Scatter(x=[input[0], neighbour['Feature1']], y=[input[1], neighbour['Feature2']],
                   mode='lines+markers', name='id {}'.format(int(neighbour['id'])))
    )
